import {saveAuxComp} from './firebase.js'
...
ComprasForm.addEventListener('submit', (e) => {
    ...
    if(document.getElementById('name').value == "" || document.getElementById('phone').value == "" || document.getElementById('email').value == "" || document.getElementById('school').value == "" || document.getElementById('experience').value == "" || document.getElementById('location').value == ""){
        ...
        const name = ComprasForm['name']
        const phone = ComprasForm['phone']
        const email = ComprasForm['email']
        const school = ComprasForm['school']
        const experience = ComprasForm['experience']
        const location = ComprasForm['location']
        const able = ComprasForm['able']

        saveAuxComp(name.value, phone.value, email.value, school.value, experience.value, location.value, able.value)
