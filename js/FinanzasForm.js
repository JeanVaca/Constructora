import {saveAuxFin} from './firebase.js'

window.addEventListener('DOMContentLoaded', () => {
    console.log('funciona');
})

const FinanzasForm = document.getElementById('finanzas-form')

FinanzasForm.addEventListener('submit', (e) => {
    e.preventDefault()
    ...
    if(document.getElementById('name').value == "" || document.getElementById('phone').value == "" || document.getElementById('email').value == "" || document.getElementById('school').value == "" || document.getElementById('experience').value == "" || document.getElementById('location').value == ""){
        ...
        const name = FinanzasForm['name']
        const phone = FinanzasForm['phone']
        const email = FinanzasForm['email']
        const school = FinanzasForm['school']
        const experience = FinanzasForm['experience']
        const location = FinanzasForm['location']
        const able = FinanzasForm['able']

        saveAuxFin(name.value, phone.value, email.value, school.value, experience.value, location.value, able.value)
