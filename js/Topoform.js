import {saveTopo} from './firebase.js'

window.addEventListener('DOMContentLoaded', () => {
    console.log('funciona');
})

const topoForm = document.getElementById('topo-form')
...
topoForm.addEventListener('submit', (e) => {
    ...
    if(document.getElementById('name').value == "" || document.getElementById('phone').value == "" || document.getElementById('email').value == "" || document.getElementById('school').value == "" || document.getElementById('experience').value == "" || document.getElementById('location').value == ""){
        ...
        const name = topoForm['name']
        const phone = topoForm['phone']
        const email = topoForm['email']
        const school = topoForm['school']
        const experience = topoForm['experience']
        const location = topoForm['location']
        const able = topoForm['able']

        saveTopo(name.value, phone.value, email.value, school.value, experience.value, location.value, able.value)
